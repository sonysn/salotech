    ...
    User.findOne({accountNumber}, (err, user) => {
        ...
        if(err || !user) {
            return res.status(401).json({
                error: "User with that account number does not exist. Please signup!"
            })
        }
         //if error or no user
        ...
        const message = "Logged in!"
         ...
         const {_id, firstName, accountNumber} = user
         console.log(user);
         return res.json({ token, user: { _id, firstName, accountNumber }, message});
